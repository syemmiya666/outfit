...
def fetch_vault_items(jwt_token, retries=2):
    ...
    for attempt in range(retries):
        try:
            resp = requests.post(url, headers=headers, data=BACKPACK_BODY, timeout=8)
...
def send_change_request(jwt_token, character_id, outfit_ids):
    ...
    resp = requests.post(url, headers=headers, data=encrypted, timeout=8)
    if resp.status_code == 200:
...
def send_emote_request(jwt_token, base_url, encrypted_payload):
    ...
    resp = requests.post(url, headers=headers, data=encrypted_payload, timeout=8)
    if resp.status_code == 200:
        return True, 200, None
    else:
        return False, resp.status_code, resp.text
...
def load_item_db():
    try:
        ...
        for it in items:
            ...
            if iid is not None:
                db[iid] = it
        print(f"Loaded {len(db)} items from {data_path}")
        return db
    except Exception as e:
